...
export default async function MyListingsPage() {
  ...
  const { data: listings } = await supabase
    .from("listings")
    .select("id, title, price_minor, currency_code, status, published_at, homepage_featured_until")
        ...
        <ul className="flex flex-col divide-y">
          {listings.map((l) => (
            <li key={l.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
              ...
              <ListingRowActions listingId={l.id} status={l.status} publishedAt={l.published_at} homepageFeaturedUntil={l.homepage_featured_until} />
            </li>
          ))}
        </ul>
      ) : (
