...
| Order | Provider | Model | Env var | Quota | Vision? |
|---|---|---|---|---|---|
| 1 (text only) | Groq | `qwen/qwen3.8-27b` | `GROQ_API_KEY` | Free — ~30 req/min, 1,000/day | No |
| 2 | Google Gemini | `gemini-flash-lite-latest` | `GOOGLE_AI_API_KEY` | Free — varies by tier, resets daily | Yes |
| 3 | GitHub Models (Copilot free tier) | `openai/gpt-4o-mini` | `GITHUB_MODELS_TOKEN` | Free — ~10 req/min, ~50/day on a free Copilot account, higher on paid plans | Yes |
| 4+ | OpenRouter (free models) | `openrouter/free` + several named free models | `OPENROUTER_API_KEY` | Free — 50/day, permanently 1,000/day once the account has ever purchased $10+ in credits | Yes (for the models used in the vision list) |
| 5 | OpenAI (direct) | `gpt-4o-mini` | `OPENAI_API_KEY` | **Paid, no free tier** — billed per request | Yes |
| 6 | OpenRouter (paid) | `anthropic/claude-sonnet-4.5` | `OPENROUTER_API_KEY` | **Paid** — the most expensive attempt, absolute last resort | Yes |
...
touching OpenRouter's much scarcer 50–1,000/day pool. Groq is skipped for photo analysis because
its free-tier models are text-only; the rest are all vision-capable. There is deliberately **no
free, unlimited ChatGPT API** in this chain — the consumer ChatGPT app and the OpenAI API are
different products, and the API has no free tier at all — so OpenAI direct is placed after every
free option (including OpenRouter's free models) but before OpenRouter's own, pricier Claude
fallback, since `gpt-4o-mini` is the cheaper of the two paid options.

Every provider here is **optional** — `buildProviderAttempts()` only adds a provider to the list if
its env var is actually set. With no keys at all, everything falls straight through to the plain
OpenRouter free→paid chain exactly as it worked before the others existed. `/admin` has a status
card showing which of Groq/Gemini/GitHub Models/OpenAI are currently configured, plus
per-provider top-up/upgrade instructions (none of them expose a live usage API the way OpenRouter's
`/api/v1/credits` does, so it's status + a guide, not live numbers).
...
1. Get a credential:
   ...
   - GitHub Models (free): a fine-grained personal access token at
   ...
   - OpenAI (paid, billing required): [platform.openai.com/api-keys](https://platform.openai.com/api-keys).
2. Add it to Vercel production: `vercel env add GROQ_API_KEY production` (or `GOOGLE_AI_API_KEY` /
   `GITHUB_MODELS_TOKEN` / `OPENAI_API_KEY`), or via the Vercel dashboard → Settings → Environment
   Variables.
