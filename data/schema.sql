...
-- Enable pgvector extension for AI-powered semantic and visual search
...
CREATE TYPE shipping_carrier AS ENUM ('budbee', 'postnl', 'dhl', 'local_pickup', 'smart_locker');
...
CREATE TABLE ads (
    ...
    title_embedding vector(1536),        -- For NLP Semantic search (e.g. OpenAI text-embedding-3-small)
    image_embedding vector(512),         -- For AI Visual Search (e.g. CLIP embeddings)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
-- HNSW Indexes for high-performance AI vector search (Cosine distance)
CREATE INDEX idx_ads_title_embedding ON ads USING hnsw (title_embedding vector_cosine_ops);
CREATE INDEX idx_ads_image_embedding ON ads USING hnsw (image_embedding vector_cosine_ops);
...
-- ============================================================================
-- AI METADATA & SMART LOCKER SYSTEM UPGRADES (BEST-EVER PLATFORM PIPELINE)
-- ============================================================================

-- 10. AI Listing Optimization Metadata (AI-First Zero-Friction)
CREATE TABLE ad_ai_metadata (
    ad_id INT PRIMARY KEY REFERENCES ads(id) ON DELETE CASCADE,
    raw_vision_analysis JSONB,           -- Complete payload from multimodal vision analysis
    confidence_score NUMERIC(5, 2),      -- Confidence rate of the automated category mapping
    detected_brand VARCHAR(100),
    detected_condition VARCHAR(50),      -- e.g. New, Like New, Good, Fair
    suggested_price_min NUMERIC(12, 2),  -- Price engine lower bracket
    suggested_price_max NUMERIC(12, 2),  -- Price engine upper bracket
    automated_tags VARCHAR(100)[],       -- Extracted search tags
    processed_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- 11. Smart Locker Shipments Ledger (Locker Escrow Flow)
...
-- Index for tracking active locker status changes
CREATE INDEX idx_locker_status ON smart_locker_shipments(status);
